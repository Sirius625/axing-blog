  ...
  <div class="blog-app">
    ...
    <div class="blog-toolbar">
      <div class="toolbar-inner">
        <div class="search-box">
          ...
          <input v-model="searchQuery" type="text" placeholder="搜索文章..." class="search-input" @input="onSearch" />
          <button v-if="searchQuery" class="clear-btn" @click="searchQuery = ''">
            <i class="fa-solid fa-times"></i>
          </button>
        </div>
        <div class="category-tabs">
          <button :class="['category-tab', { active: activeCategory === 'all' }]" @click="activeCategory = 'all'">
            <i class="fa-solid fa-layer-group"></i>
            全部
          </button>
          <button v-for="cat in categories" :key="cat" :class="['category-tab', { active: activeCategory === cat }]"
    ...
    <div class="blog-content">
      ...
      <div v-else-if="articles.length > 0" class="article-grid">
        <article v-for="article in articles" :key="article.id" class="article-card" @click="openDetail(article)">
        ...
        <div class="modal-container modal-detail">
          ...
          <div class="modal-body detail-body">
            <div class="detail-meta">
              ...
              <span class="detail-category"><i class="fa-solid fa-folder"></i> {{ detailArticle.category }}</span>
            </div>
            ...
            <div class="detail-content markdown-body" v-html="renderContent(detailArticle.content)"></div>
            <div class="detail-actions">
              <button class="action-btn like-btn" @click="handleLike(detailArticle.id)">
                <i class="fa-regular fa-thumbs-up"></i>
                点赞 {{ detailArticle.likes }}
              </button>
              <button v-if="authStore.isLoggedIn && currentUsername === detailArticle.author"
                class="action-btn delete-btn" @click="handleDelete(detailArticle.id)">
            ...
            <div class="editor-form">
              <div class="form-row">
                <div class="form-group flex-1">
                  ...
                  <input v-model="editorForm.title" type="text" placeholder="请输入文章标题" class="form-input" />
                </div>
              ...
              <div class="form-group">
                <label>摘要</label>
                <textarea v-model="editorForm.summary" placeholder="文章摘要（可选）" class="form-input form-textarea"
                  rows="2"></textarea>
              </div>
              <div class="form-group">
                <label>标签（逗号分隔）</label>
                <input v-model="editorTags" type="text" placeholder="例如: Vue, TypeScript, 前端" class="form-input" />
              </div>
              <div class="form-group">
                <label>文章内容 <span class="required">*</span></label>
                <div class="editor-tabs">
                  <button :class="['editor-tab', { active: editorTab === 'write' }]" @click="editorTab = 'write'">
                    <i class="fa-solid fa-pen"></i> 编辑
                  </button>
                  <button :class="['editor-tab', { active: editorTab === 'preview' }]" @click="editorTab = 'preview'">
                ...
                <div class="textarea-wrapper" v-if="editorTab === 'write'">
                  <textarea v-model="editorForm.content"
                    ...
                    class="form-input editor-textarea" maxlength="2000"></textarea>
                  <span class="word-count" :class="{ 'word-limit': editorForm.content.length >= 2000 }">
                    {{ editorForm.content.length }}/2000
                  </span>
                </div>
                <div v-else class="preview-area markdown-body" v-html="renderContent(editorForm.content)"></div>
              </div>
            </div>
            <div class="editor-footer">
              <button class="cancel-btn" @click="closeEditor">取消</button>
              <button class="submit-btn" :disabled="!editorForm.title || !editorForm.content" @click="submitArticle">
