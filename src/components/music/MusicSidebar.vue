<template>
  <aside class="sidebar hidden md:flex">
    <div
      ...
      @click="$emit('switchTab', 'recommend')"
    >
      <div
    ...
    <nav class="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
      <div class="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2 mt-4 px-2">
        发现音乐
      </div>
      <a
        href="#"
        @click.prevent="$emit('switchTab', 'recommend')"
        :class="{
          'bg-purple-500/20 text-purple-300': activeTab === 'recommend',
          'text-white/60 hover:bg-white/10': activeTab !== 'recommend'
        }"
      ...
      <a
        href="#"
        @click.prevent="$emit('switchTab', 'toplist')"
        :class="{
          'bg-purple-500/20 text-purple-300': activeTab === 'toplist',
          'text-white/60 hover:bg-white/10': activeTab !== 'toplist'
        }"
        class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group"
      >
      ...
      <div class="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2 mt-6 px-2">
        我的音乐
      </div>
      <a
        href="#"
        @click.prevent="$emit('switchTab', 'likes')"
        :class="{
          'bg-purple-500/20 text-purple-300': activeTab === 'likes',
          'text-white/60 hover:bg-white/10': activeTab !== 'likes'
        }"
        class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group"
      >
      ...
      <a
        href="#"
        @click.prevent="$emit('switchTab', 'history')"
        :class="{
          'bg-purple-500/20 text-purple-300': activeTab === 'history',
          'text-white/60 hover:bg-white/10': activeTab !== 'history'
        }"
        class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 group"
      >
